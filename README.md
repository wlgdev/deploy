...
Централизованная система автоматического деплоя для организации `wlgdev`: headless-приложений (боты, фоновые воркеры) и веб-сервисов за Traefik reverse-proxy.
...
3. Нажмите **Run workflow**:
   ...
   - В поле **env** впишите те же строки, что в вашем workflow (минимум `SERVICE_IMAGE_NAME` и `SERVICE_IMAGE_TAG`, иначе образ не найдётся).
   - В поле **target** впишите те же строки маршрутов, иначе деплой будет headless и сайт погаснет.
